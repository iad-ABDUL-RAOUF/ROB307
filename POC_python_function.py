...
def run_ransac(x, Hx, threshold, sample_size, goal_inliers, max_iterations, stop_at_goal=True, random_seed=None):
    ...
    while ((not(GoodEnough & stop_at_goal)) & (n_iter < max_iterations)):
      ...
      # determiner l'ensemble des points qui correspondent à ce modèle avec une
      # marge (threshold) d'erreur max 
      estimated_Hx = np.matmul(x,H.transpose())
      ...
      if np.count_nonzero(fittingPairs) >= goal_inliers :
        GoodEnough = True
        # on recalcule les modèle et le nombre d'inlier associé et on stock la
        # solution
        H_list.append(DLTcalib2(x[fittingPairs,:], Hx[fittingPairs,:])[0])
        estimated_Hx = np.matmul(x,H_list[-1].transpose())
        estimated_Hx[:,:] = estimated_Hx[:,:]/estimated_Hx[:,2:3] # TODO si j'ai le temps : prendre en compte les points infinis (estimated_Hx[i,2:3] = 0)
        err_euclide2D = np.linalg.norm(estimated_Hx[:,0:2]-Hx[:,0:2], axis = 1)
        fittingPairs = err_euclide2D < threshold
        n_inlier.append(np.count_nonzero(fittingPairs))
      n_iter = n_iter +1
